...
const GrillMenu = () => {
    const [grillsCollection, setGrillsCollection] = useState([]);
    ...
    //filtered data
    const getFilteredGrills = useMemo(() => {
        if (!query && !sortQuery) {
            return grillsCollection
                .map(grill => (
        ...
        return grillsCollection
            .filter(grill => grill.name.toLowerCase().includes(query.toLowerCase()))
            .map(grill => (
                <ItemCard key={grill.id} item={grill} />
            ));
    }, [grillsCollection, query, sortQuery]);
        ...
        <Fragment>
            ...
            <div id={styles['grid-container']}>
                ...
                <section id={styles['menu']} className={styles['grid']}>
                    {grillsCollection && getFilteredGrills}
                </section>
            </div>
        </Fragment>
    )
